function [xI,yI,CR] = IVFA_FOA(X,Y)
...
%     end
    ...
    [x_FOANew, y_FOANew, ~] = FOA(xIVFANew, yIVFANew, 1); % 一次果蝇寻优（局部）更新位置
%     [x_FOANew, y_FOANew, ~] = FOA(xI, yI, 1); % 一次果蝇寻优（局部）更新位置
    
     coverageRateImprove(t) = computeCover(x_FOANew, y_FOANew, L, rs, data); % 每次虚拟力和果蝇寻优后迭代覆盖率
...
%     end
    ...
    if t > 1 && coverageRateImprove(t) > coverageRateImprove(t - 1) 
        xI = x_FOANew; yI = y_FOANew; % 若效果优于上次迭代，则更新位置
    end
